        ...
        if (v0_0_3[i].ext !== undefined) {
          if (v0_0_3[i].ext.indexOf(" ") > 0) {
            ...
            for (j = 0; j < vals.length; j += 1) {
              ...
              assert.areEqual(mimetype.lookup(testname), v0_0_3[i].mime_type, [testname, mimetype.lookup(testname), '->', v0_0_3[i].mime_type, vals[j], "failed"].join(" "));
            }
          } else {
            testname = ["testname", v0_0_3[i].ext].join(".");
            assert.areEqual(mimetype.lookup(testname), v0_0_3[i].mime_type, [testname, v0_0_3[i].mime_type, "failed"].join(" "));
          }
        }
      });
    }
