...
var assert = require('assert'),
    ...
    module = {filename: "mongo-mimetype_test.js"};
load("./lib/v0.0.3.js");
      ...
      if (v0_0_3[i].ext !== undefined) {
        if (v0_0_3[i].ext.indexOf(" ") > 0) {
          ...
          for (j = 0; j < vals.length; j += 1) {
            ...
            assert.equal(mimetype.lookup(testname), v0_0_3[i].mime_type, [testname, mimetype.lookup(testname), '->', v0_0_3[i].mime_type, vals[j], "failed"].join(" "));
          }
        } else {
          testname = ["testname", v0_0_3[i].ext].join(".");
          assert.equal(mimetype.lookup(testname), v0_0_3[i].mime_type, [testname, v0_0_3[i].mime_type, "failed"].join(" "));
        }
      }
    };
    for (ky in v0_0_3) {
